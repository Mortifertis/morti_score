...
![CI](https://github.com/Mortifertis/morti_score/actions/workflows/ci.yml/badge.svg)
...
- [Моделирование и ограничения прогнозов](docs/modeling.md)
...
Перед первым запуском создайте runtime-конфиг из шаблона:
...
in-memory режим для локального запуска и тестов; для режима,
...
make seed      # явная загрузка seed-данных
...
app/
  ...
  bot/            опциональная demo-интеграция Telegram
...
проверить API через Swagger/OpenAPI и использовать дашборд для демонстрации
результатов. Текущая версия показывает основную бизнес-логику, работу с
данными, API и расчётными моделями, но пока не является production-ready решением.

## Идеи для дальнейшего развития

- Подключить внешний football data provider и автоматизировать импорт
  матчей.
